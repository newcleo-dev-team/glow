...
pin fuel cells. Symmetry is exploited by considering a one-third of the
complete hexagonal layout with rotational BCs on the interal borders and
translational BCs on the external borders.
"""
import os
import sys
...
export_layout_to_tdt(
    assembly,
    os.path.join(os.path.dirname(sys.argv[0]), 'test_hex_assembly_third'),
    TdtSetup(
        type_geo=LayoutGeometryType.R120,
        symmetry_type=SymmetryType.THIRD
    )
)
